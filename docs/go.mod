// The docs domain is its own Go module, nested inside the driver's.
// Hugo pins hundreds of modules, and none of them belong in the
// driver's go.sum, so the site's dependency graph lives here and
...
// install and no fetched binary, only the Go toolchain the driver
// already requires.
module github.com/liken-sh/git-csi-driver/docs
